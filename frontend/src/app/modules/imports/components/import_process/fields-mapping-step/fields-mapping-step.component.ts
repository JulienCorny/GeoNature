...
export class FieldsMappingStepComponent implements OnInit {
  ...
  populateSyntheseForm() {
    ...
    for (let entity of this.targetFields) {
      for (let theme of entity.themes) {
        for (let field of theme.fields) {
          if (!(field.name_field in controls)) {
            if (!field.autogenerated) {
            ...
            controls[field.name_field] = new FormControl(null, validators);
            controls[field.name_field].valueChanges.subscribe((value) => {
              this.onFieldMappingChange(field.name_field, value);
            });
          } else {
            controls[field.name_field].valueChanges.subscribe((value) => {
              controls[field.name_field].setValue(value, {
                onlySelf: true, emitEvent: false, emitModelToViewChange: true
                });
            });
          }
          this.syntheseForm.addControl(field.name_field, controls[field.name_field]);
        }
      }
    }
